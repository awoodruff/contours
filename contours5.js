...
d3.select('body')
  .on('mousemove', function () {
   ...
   var aspectAngle = Math.atan2(y,x) - .75 * Math.PI;
   if (aspectAngle < -2*Math.PI) aspectAngle +=2*Math.PI;
   aspectAngle = Math.abs(aspectAngle)
    //if (aspectAngle < -Math.PI) aspectAngle += Math.PI;
    console.log(x,y,Math.abs(aspectAngle) * 180/Math.PI, Math.abs(aspectAngle - Math.PI)/Math.PI)
  })
...
var twopi = 2*Math.PI;
  for (var i = 0; i<aspectData.length;i+=4) {
   ...
   aspectAngle = Math.abs(aspectAngle)
    ...
    aspectData[i] = Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+1] = Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+2] = Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+3] = 255;
  }
