...
var aspectCanvas = document.createElement('canvas');
...
aspectCanvas.width = width;
aspectCanvas.height = height;
...
var CanvasLayer2 = L.GridLayer.extend({
  createTile: function(coords){
      ...
      img.src = 'http://elevation-tiles-prod.s3.amazonaws.com/normal/'+coords.z+'/'+coords.x+'/'+coords.y+'.png'
      //tile.appendChild(img);
      return tile;
  }
});
var aspectLayer = new CanvasLayer2({attribution: '<a href="https://aws.amazon.com/public-datasets/terrain/">Elevation tiles</a> by Mapzen'}).addTo(map);

//L.tileLayer('http://elevation-tiles-prod.s3.amazonaws.com/normal/{z}/{x}/{y}.png').addTo(map)
d3.select('body')
  .on('mousemove', function () {
    var i = getIndexForCoordinates(width, d3.event.pageX, d3.event.pageY);
    console.log(aspectData[i],aspectData[i+1],aspectData[i+2],aspectData[i+3])
   // console.log(180 / Math.PI * Math.atan2(aspectData[i+1] / 255,aspectData[i] / 255))
    var x= aspectData[i] - 127;
    var y = aspectData[i+1] - 127
    var aspectAngle = -Math.atan2(y,x);
    if (aspectAngle < 0) aspectAngle += Math.PI*2;
    aspectAngle -=  1.5*Math.PI;
    if (aspectAngle < 0) aspectAngle += Math.PI*2;
    //aspectAngle = Math.abs(aspectAngle)
    //if (aspectAngle < -Math.PI) aspectAngle += Math.PI;
    console.log(x,y,aspectAngle * 180/Math.PI)
  })


var pane2 = map.createPane('aspect');
pane2.appendChild(aspectCanvas);
...
table.push( -10)
table.push(  -1)
for (i in d3.range(0, 150)){
  table.push(10 * i)
}
    
for (i in d3.range(0, 60)){
  table.push(3000 + 50 * i)
}
    
for (i in d3.range(0, 29)){
...
function getRelief(){
  ...
  aspectContext.clearRect(0,0,width,height);
  reverseTransform();
  ...
  for (var t in aspectLayer._tiles) {
    var rect = aspectLayer._tiles[t].el.getBoundingClientRect();
    aspectContext.drawImage(aspectLayer._tiles[t].el.img,rect.left,rect.top);
  }
  aspectImageData = aspectContext.getImageData(0,0,width,height);
  aspectData = aspectImageData.data;
var pi = Math.PI;
var twopi = 2*Math.PI;
  for (var i = 0; i<aspectData.length;i+=4) {
    var x= aspectData[i] - 127;
   var y = aspectData[i+1] - 127
    var aspectAngle = Math.atan2(y,x) - .75 * pi;
    if (aspectAngle < -twopi) aspectAngle += twopi;
    aspectAngle = Math.abs(aspectAngle)
    //if (aspectAngle < -Math.PI) aspectAngle += Math.PI;
    //console.log(x,y,aspectAngle * 180/Math.PI)
    aspectData[i] = 255 - Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+1] = 255 -Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+2] = 255-Math.round(255 * Math.abs(aspectAngle - pi)/pi);
    aspectData[i+3] = 100;
  }

  aspectImageData.data = aspectData;

  aspectContext.putImageData(aspectImageData, 0, 0)
...
function getAspect (coords, n) {
  ...
  // var vx = aspectData[n] - 127;
  // var vy = aspectData[n+1] - 127
  // var aspectAngle = -Math.atan2(vy,vx);
  // if (aspectAngle < 0) aspectAngle += Math.PI*2;
  // aspectAngle -=  1.5*Math.PI;
  // if (aspectAngle < 0) aspectAngle += Math.PI*2;
  // // //aspectAngle = Math.abs(aspectAngle);
